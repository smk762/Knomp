...
checkSync() {
	...
	while [ -z $blocks ]; do
		...
		if [ -z $longestchain ]; then
			...
			if [ $blocks -eq $longestchain ]; then
...
startDaemon() {
	...
	sleep 15
    checkSync $ac_name
	addresses=($(komodo-cli $acName_flag listaddressgroupings | jq -r '.[][][0]'))
	if [ ! -d ~/wallets  ]; then
		mkdir ~/wallets
	fi
	...
	num_addr=${#addresses[@]}
	echo -e "${col_blue} $num_addr addresses for $ac_name${col+default}"
	if [[ $num_addr -eq 0 ]]; then
		...
		echo -e "\e[92m Created address $address for [ $ac_name ] (info is located in ~/wallets/.${ac_name}_wallet) \e[39m"
		num_addr=1
		# clean up the json
	...
	echo -e "${col_green} Restarting $ac_name with address $address${col+default}"
	komodod $ac_params -pubkey=$pubkey > /dev/null 2>&1 
	echo -e "${col_yellow} $ac_name Restarted  ${col+default}"
}
